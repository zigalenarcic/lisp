...
obj array_remove_range(interpreter_t *interp, obj o1, i8 pos0, i8 pos1)
{
  array_t *arr = GET_ARR(o1);

  if (pos0 >= 0 && pos0 < arr->size &&
    pos1 > 0 && pos1 <= arr->size &&
    pos1 > pos0)
  {
    if (arr->element_type == ET_ANY)
    {
      for (i8 i = pos0; i < pos1; i++)
      {
        DEC(interp, NTH(o1, i));
      }
    }
    ...
    if (arr->size - pos1 > 0)
    {
      memory_copy(&GET_ARRAY_STORAGE(u1, o1)[pos0 * arr->element_size],
        &GET_ARRAY_STORAGE(u1, o1)[pos1 * arr->element_size],
        (arr->size - pos1) * arr->element_size);
    }
          ...
          case '[':
            {
              if (state->whitespace)
              {
                PARSE_ERROR("Whitespace preceeding \"[\"");
              }
              /* next parsed object - until closing bracket, should be added to aindex list */
              state->pos++;
              obj o = state->stack[state->stack_pos]; // list of current level, take last element
              // and insert (aindex _ ) around it
              if (IS_ARRAY(o) && ASIZE(o) > 0)
              {
                obj last_el = array_remove_obj(interp, o, ASIZE(o) - 1);
                obj tmp = make_array(interp, 2, interp->s_aindex, last_el);
                array_add_obj(interp, o, tmp);
                state->stack_pos++;
              ...
              {
                PARSE_ERROR("No preceding expression for \"[\"");
              }
            }
            break;
          case ']':
...
#define MAX_PRINT_ARRAY_ELEMENTS 100
  ...
  else if (IS_STRING(o))
  {
    ...
    else
    {
      ...
      i8 offset = 0;
      offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, "\"");
      char tmp[32];
      i8 res;
      do
      {
        res = convert_to_escaped(tmp, sizeof(tmp), &str);
        offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset - 1, "%s", tmp);
      }
      while (res < 0);
      offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, "\"");
  ...
  else if (IS_ARRAY(o))
  {
    ...
    if (IS_OBJ_ARRAY(o))
    {
      ...
      {
        offset += print_object_gen(mode, stream, out ? &out[offset] : NULL, out_size - offset, ((obj *)l->storage)[i], no_string_quotes);
        if (i != (l->size - 1))
          offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, " ");
      }
      if (l->size > MAX_PRINT_ARRAY_ELEMENTS)
      {
        offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, " ... %ld elements in total ...",
          l->size);
      }
      offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, ")");
    }
    else
    {
      ...
      for (i8 i = 0; i < num_print_elements; i++)
      {
        ...
        offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, "%s", tmp);
        if (i != (l->size - 1))
          offset += print_string_or_stream(mode, stream, out ? &out[offset] : NULL, out_size - offset, " ");
      }
      if (l->size > MAX_PRINT_ARRAY_ELEMENTS)
...
i8 print_object_string_no_quotes(i1 *out, size_t out_size, obj o)
{
  return print_object_gen(1, 0, out, out_size, o, 1);
}
  ...
  else if (IS_OBJ_ARRAY(o))
  {
    ...
    else
    {
      ...
      else if (sym == interp->s_aindex)
      {
        ...
        /* target */
        compile_result cr_arg0 = compile_expression(interp, target, cs, code_list, 1, -1, 0, 0);
        PROPAGATE_COMPILE_ERROR(o, &cr_arg0, &cr);
        contains_function_call |= cr_arg0.contains_function_call;
        i8 var_idx = 0;
        if (cr_arg0.type == CR_IMMEDIATE && IS_ARRAY(cr_arg0.immediate_val))
        {
          var_idx = result_to_variable(interp, cs, code_list, &cr_arg0);
        }
        else
        {
          if (!cr_arg0.type == CR_VARIABLE)
          {
            COMPILE_ERROR(target, &cr, "Only expression that can be an array is allowed");
          }
          var_idx = cr_arg0.var_idx;
        }
      ...
      else if (sym == interp->s_set)
      {
        ...
        if (IS_OBJ_ARRAY(target) && NTH(target, 0) == interp->s_aindex)
        {
          obj var_arr_idx = NTH(target, 2);
          obj var_sym2 = NTH(target, 1);
          /* array assignment */
        ...
        {
          /* lookup variables */
          i8 store_var_idx = find_lexical_variable_idx(cs, target);
          if (store_var_idx >= 0)
          {
          ...
          {
            i8 idx = find_global_variable(cs, target);
            if (idx >= 0)
            {
              i8 var_idx = result_var_idx >= 0 ? result_var_idx : allocate_temporary_variable(cs);
              compile_result cr_arg1 = compile_expression(interp, value, cs, code_list, 1, var_idx, 0, 1);
              ...
              array_add_obj(interp, code_list, make_array(interp, 3, interp->s_moveglobalr, target, MAKE_INTEGER(var_idx)));
              goto do_return;
            }
            else
            {
              COMPILE_ERROR(target, &cr, "Variable not found");
            }
          }
        }
      }
...
obj compile_function(interpreter_t *interp, obj o, compiler_state *cs_outer, compile_result *cr_out)
{
  ...
  if (cr.type != CR_ERROR)
  {
    ...
    if (interp->print_detail > 0)
    {
      FMT(STDERR_H, "Intermediate representation code:\n");
      print_object(STDERR_H, cs->ir_code);
      FMT(STDERR_H, "\n");
    }
...
i8 load_file(interpreter_t *interp, const i1* filename)
{
  ...
  {
    FMT(STDERR_H, "Parsing failed - incomplete input\n");
  }
...
INTERNAL_FUNCTION(s_copy)
{
  obj ret = GET_ARG(0);

  if (IS_ARRAY(ret))
  {
    ret = array_copy(INTERPRETER_POINTER, ret, 0);
  }

  return ret;
}

INTERNAL_FUNCTION(s_truncate)
{
  obj ret = GET_ARG(0);
  obj arg1 = GET_ARG(1);

  if (IS_ARRAY(ret) && IS_INTEGER(arg1))
  {
    array_t *arr = GET_ARR(ret);

    i8 new_size = GET_INTEGER(arg1);
    i8 previous_size = arr->size;
    ...
    return MAKE_INTEGER(previous_size);
  }
...
INTERNAL_FUNCTION(s_pop)
{
  ...
  if (IS_ARRAY(arg0))
  {
    ...
    if (NUM_ARGS > 2)
    {
      i8 pos1 = IS_INTEGER(GET_ARG(2)) ? GET_INTEGER(GET_ARG(2)) : (ASIZE(arg0) - 1);
      array_remove_range(INTERPRETER_POINTER, arg0, pos, pos1);
      return NONE_VALUE;
    }
    else
    {
...
INTERNAL_FUNCTION(s_string_print)
{
  i8 length = 0;
  for (i8 i = 0; i < NUM_ARGS; i++)
  {
    length += print_object_string_no_quotes(NULL, 0, GET_ARG(i));
  }

  obj o_ret = alloc_array(INTERPRETER_POINTER, length + 1, ET_STRING);
  i1 *text = GET_STRING_STORAGE(o_ret);

  i8 offset = 0;
  for (i8 i = 0; i < NUM_ARGS; i++)
  {
    offset += print_object_string_no_quotes(&text[offset], length + 1 /*null byte*/ - offset, GET_ARG(i));
  }

  NTH_TYPE(u1, o_ret, length) = '\0';
  array_t *l = GET_ARR(o_ret);
  l->size = length;
...
INTERNAL_FUNCTION(s_not)
{
  return IS_NONE(GET_ARG(0)) ? MAKE_INTEGER(1) : NONE_VALUE;
}
...
#define DEFINE_BIT_FUNC(name, op) \
INTERNAL_FUNCTION(name) \
{ \
  return MAKE_INTEGER(GET_INTEGER(GET_ARG(0)) op GET_INTEGER(GET_ARG(1))); \
}

DEFINE_BIT_FUNC(s_bit_and, &);
DEFINE_BIT_FUNC(s_bit_or, |);
DEFINE_BIT_FUNC(s_bit_xor, ^);
DEFINE_BIT_FUNC(s_bit_shl, <<);
DEFINE_BIT_FUNC(s_bit_shr, >>);

#undef DEFINE_BIT_FUNC
...
u0 init_interpreter(interpreter_t *interp)
{
  static const struct {
    ...
    { "load", &s_load_file, 1, 1 },
    { "to-integer", &s_to_integer, 1, 1 },
    { "int", &s_to_integer, 1, 1 },
    ...
    { "list", &s_make_list, 0, -1 },
    { "copy", &s_copy, 1, 1 },
    { "truncate", &s_truncate, 2, 2 },
    { "insert", &s_push, 2, 3 },
    { "remove", &s_pop, 1, 3 },
    { "append", &s_append, 2, 2 },
    ...
    { "print", &s_print, 1, -1 },
    { "string-print", &s_string_print, 1, -1 },
