  ...
  (with-loop
    ...
    (if (== ev-storage[0] SDL_EVENT_KEY_DOWN)
      (do 
        ...
        ;(print "Key down \n")
        (let scancode ev-storage[6])
        ;(print "Scancode " scancode "\n")

        (if (== scancode SDL_SCANCODE_Q) (quit 0))
        (if (== scancode SDL_SCANCODE_ESCAPE) (quit 0))
        (if (== scancode SDL_SCANCODE_R) (reload-shaders))

        (if (== scancode SDL_SCANCODE_UP) (set keys[0] 1))
        (if (== scancode SDL_SCANCODE_DOWN) (set keys[1] 1))
        (if (== scancode SDL_SCANCODE_LEFT) (set keys[2] 1))
        (if (== scancode SDL_SCANCODE_RIGHT) (set keys[3] 1))

        (if (== scancode SDL_SCANCODE_W) (set keys[4] 1))
        (if (== scancode SDL_SCANCODE_S) (set keys[5] 1))
        (if (== scancode SDL_SCANCODE_A) (set keys[6] 1))
        (if (== scancode SDL_SCANCODE_D) (set keys[7] 1))

        (if (== scancode SDL_SCANCODE_F) (set keys[8] 1))
        (if (== scancode SDL_SCANCODE_V) (set keys[9] 1))
        (if (== scancode SDL_SCANCODE_G) (set keys[10] 1))
        (if (== scancode SDL_SCANCODE_B) (set keys[11] 1))

        (if (== scancode SDL_SCANCODE_E) (set keys[12] 1))
        (if (== scancode SDL_SCANCODE_C) (set keys[13] 1))

        (if (== scancode SDL_SCANCODE_N) (set keys[14] 1))
        (if (== scancode SDL_SCANCODE_M) (set keys[15] 1))
        ))
    ...
    (if (== ev-storage[0] SDL_EVENT_KEY_UP)
      (do 
        ...
        (if (== scancode SDL_SCANCODE_UP) (set keys[0] 0))
        (if (== scancode SDL_SCANCODE_DOWN) (set keys[1] 0))
        (if (== scancode SDL_SCANCODE_LEFT) (set keys[2] 0))
        (if (== scancode SDL_SCANCODE_RIGHT) (set keys[3] 0))

        (if (== scancode SDL_SCANCODE_W) (set keys[4] 0))
        (if (== scancode SDL_SCANCODE_S) (set keys[5] 0))
        (if (== scancode SDL_SCANCODE_A) (set keys[6] 0))
        (if (== scancode SDL_SCANCODE_D) (set keys[7] 0))

        (if (== scancode SDL_SCANCODE_F) (set keys[8] 0))
        (if (== scancode SDL_SCANCODE_V) (set keys[9] 0))
        (if (== scancode SDL_SCANCODE_G) (set keys[10] 0))
        (if (== scancode SDL_SCANCODE_B) (set keys[11] 0))

        (if (== scancode SDL_SCANCODE_E) (set keys[12] 0))
        (if (== scancode SDL_SCANCODE_C) (set keys[13] 0))

        (if (== scancode SDL_SCANCODE_N) (set keys[14] 0))
        (if (== scancode SDL_SCANCODE_M) (set keys[15] 0))
        ))
